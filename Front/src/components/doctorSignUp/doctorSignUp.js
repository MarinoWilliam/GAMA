...
const model = Schema.Model({
  ...
  clinicLocation: Schema.Types.StringType().isRequired('This field is required.'),
  gender: Schema.Types.StringType().isRequired('This field is required.'),
  email: Schema.Types.StringType().isEmail('Please enter a valid email address.'),
  speciality: Schema.Types.StringType().isRequired('This field is required.'),
...
const TownData = ['Maadi', 'Helwan', 'Smouha', 'Mansoura', 'Zagazig', 'Benha', 'Sadat', '6th October', 'Giza'].map(
  item => ({ label: item, value: item })
);
...
const DoctorSignUp = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      let res = await fetch("http://localhost:8000/doctorSignup", {
        method: "POST",
  ...
  return (
    <div>
      ...
      <p className='signPara'>Have a place to hold all your medical data</p>
      <br />
      <Form
        ref={formRef}
      ...
      >
        ...
        <TextField name="clinicLocation" label="Clinic Location" />
        ...
        <Field
          ...
          label="Speciality"
          accepter={SelectPicker}
          data={SpecialityData} placeholder="Ex. Dentistry"
        />

...
const PatientSignIn = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      let res = await fetch("http://localhost:8000/patientLogin", {
        method: "POST",
      ...
      if ((res.status === 200) && (resJson.loginFailed === 'false')) {
        ...
        window.location.replace(`/patientHome/${resJson.id}`);
      } else {
  ...
  return (
    <div>
      <h1 className="signHeader">Patient Account</h1>
      <br />
      ...
      >
        {loginMessage}
        <TextField name="username" label="Email" block/>
        <TextField name="password" label="Password" type="password" autoComplete="off" block/>
        <ButtonToolbar>
